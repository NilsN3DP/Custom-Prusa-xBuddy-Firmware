 ...
 *#### Usage
 *
 *    M152 L[target] [ R | B | G | H | S | V | D | T | A | P ]
 *
 *#### Parameters
 *
 * - `L` - Light target: 0 display/status LEDs, 1 bed LEDs, 2 chamber LEDs
 * - `D` - Duration in milliseconds for temporary targets. Bed and chamber default to 300000 ms.
 *
 * Display forwards to the M150 custom animation handler.
 * Chamber forwards to the M151 side strip handler when available.
 * Bed applies a temporary override and then falls back to the printer status color.
 */
void PrusaGcodeSuite::M152() {
    ...
    case 1:
#if HAS_XBUDDY_EXTENSION() && XBUDDY_EXTENSION_VARIANT_IS_STANDARD()
        buddy::xbuddy_extension().set_bed_leds_override(*color, parser.ulongval('D', 300000));
#endif
        break;
    ...
    case 2:
...
#if HAS_XBUDDY_EXTENSION() && XBUDDY_EXTENSION_VARIANT_IS_STANDARD()
        {
            ...
            const uint32_t duration = parser.ulongval('D', 300000);
            if (duration == 0) {
                buddy::xbuddy_extension().set_chamber_leds_percent(buddy::XBuddyExtension::led_pwm2pct(white));
            } else {
                buddy::xbuddy_extension().set_chamber_leds_override(white, duration);
            }
        }
#endif
        break;
...
void PrusaGcodeSuite::M153() {
    ...
    const auto pwm_target = speed < 0 ? pwm_auto : buddy::XBuddyExtension::FanPWM::from_percent(speed);
