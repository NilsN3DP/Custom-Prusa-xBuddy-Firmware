...
struct Session {
    ...
    void evaluate_window() {
        ...
        const auto current_x = input_shaper::get_axis_config(X_AXIS);
        const auto current_y = input_shaper::get_axis_config(Y_AXIS);
        const bool changed_x = apply_axis(X_AXIS, current_x, analyzer_x.detected_peak());
        const bool changed_y = apply_axis(Y_AXIS, current_y, analyzer_y.detected_peak());
...
void step() {
    const bool enabled = config_store().adaptive_input_shaper_enabled.get();
    if (!enabled || !marlin_server::is_printing()) {
        stop_session();
        return;
    }
