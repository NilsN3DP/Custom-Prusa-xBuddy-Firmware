...
static AccentColor preset_accent_for(ThemePreset p) {
    ...
    case ThemePreset::light_mint:
        return AccentColor::filament_turquoise;
    case ThemePreset::dark_mint:
        return AccentColor::bright_turquoise;
    case ThemePreset::prusa_classic:
        return AccentColor::prusa_orange;
    case ThemePreset::oled_dark:
        return AccentColor::white;
    case ThemePreset::graphite_cyan:
        return AccentColor::bright_turquoise;
    case ThemePreset::triforce_one:
        return AccentColor::triforce_gold;
    case ThemePreset::custom:
    default:
        return config_store().ui_accent_color.get();
    }
}
...
static BackgroundColor preset_background_for(ThemePreset p) {
    ...
    case ThemePreset::dark_mint:
        return BackgroundColor::dark_gray;
    case ThemePreset::prusa_classic:
    ...
    case ThemePreset::graphite_cyan:
        return BackgroundColor::dark_gray;
    case ThemePreset::triforce_one:
        return BackgroundColor::triforce_green;
    case ThemePreset::custom:
    default:
        return config_store().ui_background_color.get();
    }
}

// ---------------------------------------------------------------------------
// In-memory theme state — always initialized to light_mint defaults.
// Avoids showing wrong colors if the flash store has stale/unexpected values.
// Updated by set_* functions when the user explicitly changes settings.
// ---------------------------------------------------------------------------
...
static void set_accent_impl(AccentColor a) {
    ...
    s_accent_enum = a;
    s_accent = accent_color_for(a,
        config_store().ui_custom_hue.get(),
        config_store().ui_custom_saturation.get(),
        config_store().ui_custom_value.get());
...
// ---------------------------------------------------------------------------
// Setters — update in-memory state AND persist to store
// ---------------------------------------------------------------------------
...
void set_custom_hue(uint16_t hue) {
    ...
    if (s_accent_enum == AccentColor::custom) {
        const Color old_accent = s_accent;
        s_accent = accent_color_for(AccentColor::custom, hue,
            config_store().ui_custom_saturation.get(),
            config_store().ui_custom_value.get());
...
void set_custom_saturation(uint8_t saturation) {
    ...
    if (s_accent_enum == AccentColor::custom) {
        const Color old_accent = s_accent;
        s_accent = accent_color_for(AccentColor::custom,
            config_store().ui_custom_hue.get(), saturation,
            config_store().ui_custom_value.get());
        if (old_accent != s_accent) {
            bump_generation();
        }
...
void set_custom_value(uint8_t value) {
    ...
    if (s_accent_enum == AccentColor::custom) {
        ...
        s_accent = accent_color_for(AccentColor::custom,
            config_store().ui_custom_hue.get(),
            config_store().ui_custom_saturation.get(), value);
        if (old_accent != s_accent) {
            bump_generation();
        }
    }
...
// ---------------------------------------------------------------------------
// Icon accent remapping
// ---------------------------------------------------------------------------

bool is_source_icon_accent(uint8_t r, uint8_t g, uint8_t b) {
    // Home/footer assets contain antialiased accent pixels, so the match has
    // to include the brighter turquoise edge tones, not only the exact source.
...
Color remap_source_icon_accent(uint8_t r, uint8_t g, uint8_t b) {
    // Reference luma for orange (248,101,27), turquoise (0,143,120), and
    // TriForce gold (214,166,50) source icons.
    constexpr uint32_t ref_luma_orange     = (77u * 248 + 150u * 101 + 29u *  27) >> 8; // ≈136
    constexpr uint32_t ref_luma_turquoise  = (77u *   0 + 150u * 143 + 29u * 120) >> 8; // ≈97
    constexpr uint32_t ref_luma_gold       = (77u * 214 + 150u * 166 + 29u *  50) >> 8; // ≈153
    const uint32_t pixel_luma = (77u * r + 150u * g + 29u * b) >> 8;
    const uint32_t ref_luma = (r > g)
        ? ((g > 130) ? ref_luma_gold : ref_luma_orange)
        : ref_luma_turquoise;
    const Color acc = s_accent;
    auto scale = [&](uint8_t ch) -> uint8_t {
        const uint32_t result = ref_luma > 0
            ? (static_cast<uint32_t>(ch) * pixel_luma + ref_luma / 2) / ref_luma
